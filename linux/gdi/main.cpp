	...
	if (!binit){
		...
		if (nmissing > 0) {
			/*for(int i=0; i < nmissing; i++){
					fprintf(stderr, "%s: %s", "program_name", missing[i]);
						fprintf(stderr, "%s: The string %s will be used in place","program_name", def_string);
								}*/
...
int newWindow(int tp,int x, int y, int w, int h, wchar_t* iconPath){
	...
	if (iconPath != NULL){
		...
		while (token != NULL){
			...
			if (appln>0){
				if (length==0){
					buffer = malloc(appln*sizeof(unsigned long));
				}else{
...
int main(int argc, char **argv){
	...
	int id = newWindow(WINDOW_TYPE_NORMAL_NOT_RESIZABLE,0,0,400,300, L"/home/webdel/Sviluppo/dwservice/dwagent/images/logo16x16.xpm\n/home/webdel/Sviluppo/dwservice/dwagent/images/logo32x32.xpm");
