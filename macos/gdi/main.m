...
- (void)insertText:(id)string{
    ...
    for (i=0;i<n;i++){
    	...
    	//NSLog(c);
    	const char* temp = [c cStringUsingEncoding:NSUTF32LittleEndianStringEncoding];
...
void setTitle(int id, wchar_t* title){
	...
	if (dwa!=NULL){
		NSWindow *window = [dwa win];
		NSString* nsTitle = [[NSString alloc] initWithBytes:title length:wcslen(title)*sizeof(*title) encoding:NSUTF32LittleEndianStringEncoding];
...
int getTextWidth(int id,wchar_t* str){
	...
	if (dwa!=NULL){
		...
		NSString* nsStr = [[NSString alloc] initWithBytes:str length:wcslen(str)*sizeof(*str) encoding:NSUTF32LittleEndianStringEncoding];
