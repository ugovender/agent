...
GRADIENT_DIRECTION_TOPBOTTON=2;
...
def is_windows_user_in_admin_group():
    if is_windows():
        return gdw_lib().isUserInAdminGroup()
    else:
        raise Exception("invalid os.")

def is_windows_run_as_admin():
    if is_windows():
        return gdw_lib().isRunAsAdmin()
    else:
        raise Exception("invalid os.")

def is_windows_process_elevated():
    return gdw_lib().isProcessElevated()    
        ...
        if get_arch()=="x86":
            if is_windows():
                ...
                if is_os_32bit():
                    ...
                    namelibinst="dwaggdi_x86_64.so"
            if os.path.exists(namelibinst): #Installer Mode
                gdwlib = ctypes.CDLL("." + os.sep + namelibinst)
            elif os.path.exists("native" + os.sep + namelib):
                gdwlib = ctypes.CDLL("native" + os.sep + namelib)
            else: #Develop Mode
                if is_os_32bit():
                    gdwlib = ctypes.CDLL("native_" + pathlib + "_x86_32" + os.sep + namelib)
                elif is_os_64bit():
                    gdwlib = ctypes.CDLL("native_" + pathlib + "_x86_64" + os.sep + namelib)
...
def init_window(win):
    if win._id is None:
        logo=None
        if is_windows():
            logo=u"images\\logo.ico"
        elif is_linux():
            logo=u"images/logo16x16.xpm\nimages/logo32x32.xpm"
...
class Scheduler():
    ...
    def add(self,wt,func,*args, **kargs):
        ...
        try:
            if self._destroy:
                return
            itm={"time": long(time.time() * 1000),"wait":wt*1000, "func":func, "args":args ,"kargs":kargs}
...
class Window:
    def __init__(self,tp=WINDOW_TYPE_NORMAL_NOT_RESIZABLE):
        self._id=None;
        self._type=tp
        self._top_windows=[]
        self._title="";
...
class DialogMessage(Window):
    ...
    def __init__(self,act,lv,prnt=None):
        if prnt is None:
            Window.__init__(self,WINDOW_TYPE_DIALOG)
        else:
            if prnt.is_show():
                Window.__init__(self,WINDOW_TYPE_TOOL)
            else:
                Window.__init__(self,WINDOW_TYPE_DIALOG)
        self._actions=act
        self._level=lv
        self._message=u""
        self._action=None
        self._parent_window=prnt
        if self._parent_window is not None:
            self._parent_window._top_windows.append(self)
            self.set_title(prnt.get_title())
    ...
    def _draw_background_gradient(self,pobj,x,y,w,h):
        ...
        if self._gradient_direction==GRADIENT_DIRECTION_LEFTRIGHT:
            ...
            for i in range(w):
                ...
                gstart+=gstep
                bstart+=bstep
        elif self._gradient_direction==GRADIENT_DIRECTION_TOPBOTTON:
...
def _test_altra_finestra(e):
    prnt=None
    if "window" in e:
        prnt=e["window"]
    wwn = DialogMessage(DIALOGMESSAGE_ACTIONS_YESNO,DIALOGMESSAGE_LEVEL_WARN,prnt) 
    wwn.set_title("AA")
    wwn.set_message(u"Prova Messaggio.\nQuesto è un messaggio di prova. Oggi è una bella giornata calda ma non troppo già. " + str(e["action"]))
    wwn.show()
...
if __name__ == "__main__":
    ww = Window()
    ww.set_title(u"Titolo")
    ww.set_action(_test_window_action);
    #ww.set_position(100, 100)
    ww.set_size(800, 500)
    ww.set_show_position(WINDOW_POSITION_CENTER_SCREEN)
    ...
    rr = RadioButton()
    rr.set_text(resources.get_message("yes"))
    rr.set_group("PROVA");
    rr.set_position(250, 170)
    ww.add_component(rr)
    
    rr = RadioButton()
    rr.set_text(resources.get_message("no"))
    rr.set_group("PROVA");
    ...
    pl.set_background_gradient("83e5ff", "FFFFFF", GRADIENT_DIRECTION_LEFTRIGHT)
    ww.add_component(pl)
    ...
    b2.set_action(_test_altra_finestra)
    pb.add_component(b2)    
   
    #ww.show_notifyicon(u"images\\logo.ico",u"CIAO")
